...
file<-list.files(here::here("pilot_data"), pattern = ".csv") # get files 

setwd(here::here("pilot_data"))
...
rt1<- rt %>%
  filter(maintrials.ran==1)%>%
  select(participant, string, blur, study, lex, corr, rt) %>%
  mutate(lex=ifelse(lex=="y", "word", "nonword"))
#mutate(rt_win=datawizard::winsorize(rt))

rt1

accuracy<-rt1 %>%
  dplyr::group_by(participant)%>%
  dplyr::summarise(accuray=mean(acc, na.rm=TRUE))
